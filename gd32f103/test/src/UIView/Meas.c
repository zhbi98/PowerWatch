...
void measUpdate()
{
    ...
    } else if (displayMode.cont1_mode == 1) {
    ...
    } else if (displayMode.cont1_mode == 2) {
    ...
    memset(buf, '\0', 10);
    fmt = param_fmt(average_calc.average);
    index = get_param_fmt(fmt);
    snprintf(buf, 10, fmt_str[index], fmt);
    lv_label_set_text_fmt(measSence.mainShow.lableValue2, "%s", buf);
    if (displayMode.cont2_mode == 0)
        lv_label_set_text_fmt(measSence.mainShow.lableUnit2, "%s", unit_str[0][get_param_unit(average_calc.average)]);
    else if (displayMode.cont2_mode == 1)
        lv_label_set_text_fmt(measSence.mainShow.lableUnit2, "%s", unit_str[1][get_param_unit(average_calc.average)]);
    else if (displayMode.cont2_mode == 2)
        lv_label_set_text_fmt(measSence.mainShow.lableUnit2, "%s", unit_str[2][get_param_unit(average_calc.average)]);
