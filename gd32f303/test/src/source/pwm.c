...
void pwm_gpio_init()
{
    rcu_periph_clock_enable(PWM_CLOCK);
	gpio_pin_remap_config(GPIO_TIMER1_FULL_REMAP, ENABLE);
    gpio_init(PWM_GPIO, GPIO_MODE_AF_PP, GPIO_OSPEED_50MHZ, PWM_PIN);
}
...
void timers_init()
{
    ...
    timer_channel_output_config(TIMER, TIMER_CH, &timer_out_init);

    timer_channel_output_pulse_value_config(TIMER, TIMER_CH, period_cnt / 2);
...
void pwm_out(unsigned int pwmv)
{
    timer_channel_output_pulse_value_config(TIMER, TIMER_CH, pwmv);
}
