...
void led_controller_handler(struct led_obj_t * led)
{
...
void led2_controller_cb(unsigned char status)
{
...
void led1_set_status(unsigned char status)
{
    if (status)
        gpio_bit_set(LED1_GPIO, LED1_PIN);
    else
        gpio_bit_reset(LED1_GPIO, LED1_PIN);
}

void led2_set_status(unsigned char status)
{
    if (status)
