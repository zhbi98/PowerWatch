...
static nt_acct_t * nt_master_find_account(
    ...
    lv_ll_t * acct_list, const int8_t * acct_id)
{
    ...
    _LV_LL_READ_BACK(acct_list, _acct_p) {
...
uint8_t nt_master_add_acct(nt_master_t * master_p, nt_acct_t * acct_p)
{
    ...
    if (nt_master_find_account(master_p, acct_p->acct_id) != NULL)
        return NT_MASTER_INV;

    /*Add the account to the account list*/
    nt_acct_t * account = _lv_ll_ins_head(&master_p->accounts_ll);
...
uint8_t nt_master_remove_acct(nt_master_t * master_p, nt_acct_t * acct_p)
{
    ...
    if (nt_master_find_account(master_p, acct_p->acct_id) != NULL) {
        ...
        _lv_ll_remove(&master_p->accounts_ll, acct_p);
        lv_mem_free(acct_p);
